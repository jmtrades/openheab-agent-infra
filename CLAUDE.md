...
| Metric | Value |
|---|---|
| Primitive modules | **181** in `src/primitives/` |
| HTTP routes | **1427+** registered |
...
**L29 Design + workflows + adapters + CS + i18n + incidents (6):** design_system (central CSS tokens + JSON tokens + reusable component classes + Cmd+K command palette + dark/light auto via prefers-color-scheme + WCAG 2.2 AA-friendly focus rings + prefers-reduced-motion), workflow_builder (Zapier-style trigger→action: 11 trigger kinds + 16 action kinds + scheduled+webhook+manual triggers + run history), provider_adapters (32 external providers cataloged with exact env-var manifest, configured-status endpoint, call log — Stripe/Stripe-Issuing/Modern-Treasury/Plaid/Wise/Twilio/SendGrid/Onfido/Persona/Sumsub/Comply-Advantage/Anthropic/OpenAI/Google-AI/Mistral/Together/Modal/E2B/Browserbase/Vercel/Cloudflare/AWS-S3/GitHub/Sentry/Datadog/PagerDuty/Slack/Discord/Vanta/Drata/Carta/Alchemy), customer_success (5-axis health scoring + churn risk + NPS surveys + QBR pack generator + expansion-opp flagging + Customer Advisory Board mgmt), i18n (20 locales + 18 currencies with proper formatting + locale auto-detect + per-agent locale storage + translation table API), status_incidents (real incident management replacing static /status: 10 components, 5 severities, multi-update incident threads, subscriber notifications, RSS feed at /status/incidents.rss, live HTML at /status/live)
